...
def extract_activations(
    ...
    seq = t.tensor(
        [tokens + [1] * (max_tokens - len(tokens)) for tokens in token_list], device=model.device
    )
    ...
    for b in range(0, len(statements), batch_size):
        ...
        for layer in layers:
            for statement in range(min(b + batch_size, len(statements)) - b):
                result[layer].append(
                    output.hidden_states[layer + 1][statement, len(current_tokens[statement]) - 1]
                )
    for layer in layers:
...
for name in DATASET_NAMES:
    ...
    labs = t.tensor(df["label"].values, dtype=t.float32, device=device)
    statements_dict[name] = statements

    acts = extract_activations(statements, model, tokenizer, [PROBE_LAYER])
    activations[name] = acts[PROBE_LAYER].float()
...
def layer_sweep_accuracy(
    ...
    if use_cached:
        ...
        activations_train = extract_activations(
            statements_train, model, tokenizer, layers, batch_size
        )
        cached_activations_train = activations_train
        activations_test = extract_activations(
            statements_test, model, tokenizer, layers, batch_size
        )
...
class MMProbe(t.nn.Module):
    def __init__(
        ...
        if covariance is not None:
            self.inv_covariance = t.nn.Parameter(
...
class LRProbe(t.nn.Module):
    ...
    def from_data(
        ...
        lr_model = LogisticRegression(C=C, fit_intercept=False, random_state=42, max_iter=1000)
        lr_model.fit(X_scaled, labels.cpu().float().numpy())
        ...
        # lr.fit((acts.numpy() - ss.mean_) / ss.scale_, labels.cpu().float().numpy())
        result = LRProbe(acts.shape[-1], scaler_mean, scaler_scale).to(device)
        result.net[0].weight.data[0] = t.tensor(lr_model.coef_[0], dtype=t.float32).to(device)
        return result
        ...
        if intervention != "none":
            ...
            def make_batch_hook(dir_vec, attn_mask, scl):
                def hook_fn(module, input, output):
                    ...
                    indices = t.arange(0, len(batch))
                    hidden_states[indices, end - len_suffix - 1, :] += scl * dir_vec
                    hidden_states[indices, end - len_suffix, :] += scl * dir_vec
...
neg_cities_labels = t.tensor(neg_cities_df["label"].values, dtype=t.float32, device=device)
...
combined_probe = MMProbe.from_data(combined_acts, combined_labels).to(device)
...
lr_combined = LRProbe.from_data(combined_acts, combined_labels).to(device)
...
def construct_instructed_pairs(
    ...
    for fact in tqdm(facts, desc="Extracting instructed-pairs"):
        ...
        for sys_prompt, acts_dict in [(honest_sys, honest_acts), (dishonest_sys, dishonest_acts)]:
            ...
            activations = ChatActivations.from_messages(messages, tokenizer)
            acts = activations.extract_activations(model, layers, average=True)
            for layer in layers:
                acts_dict[layer].append(acts[layer])

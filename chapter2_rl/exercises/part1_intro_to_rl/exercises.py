...
class Cheater(Agent):
    def __init__(
        ...
        self.optimal = find_optimal_policy(env.unwrapped.env, gamma)
...
curves = utils.seeded_curves(
    norvig_specs, "NorvigGrid-v0", gamma=gamma, n_runs=n_runs, n_seeds=n_seeds
)
...
class SARSA_lambda(SARSA):
    ...
    def observe(self, exp: Experience):
        s_t, a_t, r_t, s_t_1, a_t_1 = exp.obs, exp.act, exp.reward, exp.new_obs, exp.new_act
        delta_t = r_t + self.gamma * self.e[s_t_1, a_t_1] - self.Q[s_t, a_t]
        self.e[s_t, a_t] += 1
        self.Q += self.config.lr * delta_t * self.e
        self.e = self.gamma * self.config.lambda_ * self.e
...
# average over a few seeds (tabular RL is noisy) and plot mean ± standard error
curves = utils.seeded_curves(
    norvig_lambda_specs, "NorvigGrid-v0", gamma=gamma, n_runs=n_runs, n_seeds=n_seeds
)
...
class RewardAveraging(Agent):
    ...
    def reset(self, seed: int):
        ...
        self.Q = np.full(num_arms, self.optimism, dtype=float)
